/**
 * AI integration via OpenRouter (GPT-5.4).
 *
 * File kept as lib/gemini.js to preserve import paths across all API routes.
 * All Gemini SDK code has been replaced with OpenRouter chat completions.
 */
...
async function callOpenRouter(messages, { jsonMode = false, maxTokens = 16384 } = {}) {
  ...
  const t0 = Date.now();
  console.log(`[AI] Calling ${MODEL}, maxTokens=${body.max_tokens}, jsonMode=${jsonMode}`);
  ...
  console.log(`[AI] Response status ${res.status} — ${Date.now() - t0}ms`);
  ...
  console.log(`[AI] Content received, length=${content?.length || 0} — ${Date.now() - t0}ms`);
...

{
  ...
  "financialImpact": {
    ...
    "estimatedValueChange": "Description of how the contract value changed",
...
export async function analyzeTender(documentText, rfxType = "rfp") {
  try {
    const { getRfxPrompt, RFX_SYSTEM_PROMPTS } = await import("@/lib/rfx-prompts");
    const systemPrompt = RFX_SYSTEM_PROMPTS[rfxType] || RFX_SYSTEM_PROMPTS.rfp;
    const typePrompt = getRfxPrompt(rfxType);

    // RFP uses the existing comprehensive ANALYSIS_PROMPT; other types use specialized prompts
    const userPrompt = typePrompt || ANALYSIS_PROMPT;
    ...
    // Tag the result with its type so the frontend knows which layout to render
    analysis.rfxType = rfxType;
...
/**
 * Extract text from a PDF buffer using unpdf (serverless-safe, no worker needed).
 * unpdf bundles pdfjs internally with no web-worker or canvas dependency.
 */
...
export async function analyzeTenderFromPDF(base64PDF, rfxType = "rfp") {
  try {
    ...
    if (!text || text.trim().length < 20) {
      console.log("[PDF] Text too short — likely scanned/image PDF");
      return {
        success: false,
        error: "This PDF appears to be scanned or image-based and could not be read as text. Please upload a text-based PDF, DOCX, or TXT file instead.",
      };
    }

    return analyzeTender(text.substring(0, 100000), rfxType);
  } catch (error) {
    console.error("[PDF] Extraction failed:", error.name, error.message);
    console.error("[PDF] Stack:", error.stack);

    const msg = error.message || "";
    if (msg.includes("password") || msg.includes("Password")) {
      return { success: false, error: "This PDF is password-protected. Please upload an unprotected file." };
    }
    if (msg.includes("Invalid PDF") || msg.includes("Bad PDF")) {
      return { success: false, error: "This file does not appear to be a valid PDF. Please check the file and try again." };
    }

    return {
      success: false,
      error: "Failed to read PDF content. The file may be corrupted or in an unsupported format.",
    };
  }
}
...
export async function generateProposalSection(analysisData, sectionType) {
  try {
    ...
    const content = await callOpenRouter([
      { role: "system", content: "You are an expert bid proposal writer. Write in professional third-person proposal language." },
    ...
    return {
      success: false,
      error: "Failed to generate proposal section. Please try again.",
    };
  }
}
...
export async function compareTenderAmendments(originalText, amendedText) {
  try {
    const content = await callOpenRouter(
      [
        { role: "system", content: "You are an expert tender amendment analyst. Return ONLY valid JSON." },
        {
          role: "user",
    ...
    const comparison = parseJSONResponse(content);
    return { success: true, data: comparison };
  } catch (error) {
    console.error("AI comparison error:", error);
    return {
      success: false,
      error: "Failed to compare tender documents. Please try again.",
    };
  }
}
...
export async function compareTenderPDFs(base64Original, base64Amended) {
  try {
    const [originalText, amendedText] = await Promise.all([
      extractTextFromPDF(Buffer.from(base64Original, "base64")),
      extractTextFromPDF(Buffer.from(base64Amended, "base64")),
    ]);

    if (!originalText.trim() || !amendedText.trim()) {
      return {
        success: false,
        error: "Could not extract text from one or both PDFs. They may be scanned or image-based.",
      };
    }

    return compareTenderAmendments(
      originalText.substring(0, 80000),
      amendedText.substring(0, 80000)
    );
  } catch (error) {
    console.error("[PDF compare] Extraction failed:", error.name, error.message);
    return {
      success: false,
      error: "Failed to read PDF content for comparison. Please try different files.",
    };
  }
}
